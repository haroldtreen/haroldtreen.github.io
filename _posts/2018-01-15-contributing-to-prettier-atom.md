...
The main way to use it is with the `prettier-cli`.
...
This also helps me be more efficient as I code. Rather than spending time writing out spaces and brackets, I can just type out a bunch of code on one line and prettier will nicely format it for me on save.

`prettier-atom` also integrates with [eslint](https://eslint.org/). This means that in addition to applying the prettier on save, it can also fix linting issues.

It does that by running the output of prettier through `eslint --fix`.

This helps reduce the number of linting issues I run into when I go to commit.
...
I had been noticing some lag with atoms boot, so I opened up Timecop to see what might be causing the issue:
...
The maintainer replied a few days later saying he'd done a bunch of things to reduce activation time, but were out of ideas. Receiving a reply that showed a willingness to consider my ideas gave me the necessary encouragement to dig deeper.
...
Atom is basically a web application, so it has all the same debug tooling as Chrome. The article also mentioned a `--profile-startup` flag that could be used to collect debug information for the initialization stage.
...
By hovering over these bars, I was able to find the one corresponding to the `prettier-atom.activate` function and understand what was causing the slow activation time.
...
* Another file was requiring `lodash/fp` which was taking 111ms to run.
...
If we could avoid loading these packages, we would save ~170ms from the activation time!
...
const getAllScopes = () =>
	[
		...
		getGraphQlScopes(),
		getMarkdownScopes()
	].reduce((allScopes, scopes) => allScopes.concat(scopes), []);
```
...
This is nice for keeping track of what's required, but slows down the initial load of the file. Anything that requires the above file will need to wait for `lodash` to load before it can even see the `reverseAndCompact` function.

This is especially annoying if we want to use the `activate` function. It doesn't depend on `lodash`, but still needs to wait for `lodash` if we import it.
...
* To commit you use `yarn start commit`. This starts a little process to describe your commit. The command ensures that all commits conform to a consistent style and make things like generating a `CHANGELOG` easier.
...
* **Profile your code:** It's amazing the optimizations you can find if you just dig a little. You can't understand the performance of your code by simply reading it. You need to run it and analyze it. I never would have guessed imports would account for 75% of startup time.
...
* **Babel has a cost:** Writing code with the latest ES6 features is all the craze, but there's not much discussion regarding the cost of doing so. Simple things like using the `...` operator might require importing heavy dependencies that will slow things down. How much are you willing to slow down your code to use the new fancy syntax?
* **Open source is a great way to learn about project management:** Seeing the amount of tooling and documentation that existed for `prettier-atom` was really neat. Having a command to update your contributors list? For generating a release? For creating consistent commits? Moving all your packages into a separate file? These are things I'd never really experienced. It's a great lesson in making projects beginner friendly and easy to setup. These things are easy to gloss over, but provide a lot of value in the long run.
